  ...
  if (inviteCode) {
    const inviteLink =
      typeof window !== "undefined"
        ...
        : `/i/${inviteCode}`;
    const shareText = `Hi! I'd like you to try Diya. Open this link to get started: ${inviteLink}`;

    return (
      <div className="rounded-2xl border border-line bg-bg-elevated p-6">
        <p className="font-display text-2xl text-ink">Their invite link is ready</p>
        <p className="mt-2 text-[17px] text-ink-muted">
          Send this to them on WhatsApp. Save it too — you&apos;ll need it if they lose the
          message.
        </p>
        ...
        <a
          ...
          className="mt-4 flex h-14 w-full items-center justify-center rounded-full bg-accent text-[17px] font-medium text-bg"
        >
          Send on WhatsApp
        </a>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="flex flex-col gap-2">
          ...
          <input
            name="caregiverName"
            required
            className="h-14 rounded-xl border border-line bg-bg-elevated px-4 text-[17px] text-ink outline-none placeholder:text-ink-muted/60 focus:border-accent"
        ...
        <label className="flex flex-col gap-2">
          ...
          <select
            ...
            required
            defaultValue=""
            className="h-14 rounded-xl border border-line bg-bg-elevated px-4 text-[17px] text-ink outline-none focus:border-accent"
          >
      ...
      <button
        ...
        className="mt-1 flex h-14 w-full items-center justify-center rounded-full bg-accent text-[17px] font-medium text-bg disabled:opacity-60 sm:w-auto sm:px-10"
      >
        {status === "submitting" ? "Getting their link…" : "Get their invite link"}
      </button>

      {status === "error" && (
        <p className="text-[15px] text-[#e0765a]">That didn&apos;t go through. Try again.</p>
      )}
    </form>
  );
}
